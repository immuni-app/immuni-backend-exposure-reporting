...
def generate_random_batch_eu(
    index: int, num_keys: int, period_start: datetime, period_end: datetime, origin: str
) -> None:
    ...
    BatchFileEu(
        index=index, keys=keys, period_start=period_start, period_end=period_end, origin=origin
    ).save()


def create_random_batches_eu(
